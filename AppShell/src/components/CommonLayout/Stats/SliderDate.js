        ...
        >
          {
            dates.map((date, i) => {
              return (
                <SwiperSlide >
                  <div className={`flex justify-center w-fit rounded-2xl p-[1px] ${i === 2 ? 'primary-linear-gr-bg ' : ''}`}>
                    <div className={`flex flex-col gap-1 items-center rounded-2xl ${i === 2 ? 'bg-black-primary w-[54px] h-[70px]' : 'bg-white  w-[54px] h-[60px] '} p-[0.6rem]`}>
                      <div className='poppins-bold'>{date.day}</div>
                      <div className={`poppins-regular text-xs  ${i === 2 ? '' : 'text-gray-600'}`}>{date.dayName}</div>
                      {i == 2 ? <div className='mt-1'>
                        <svg width="22" height="3" viewBox="0 0 22 3" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect x="0.285889" width="21" height="3" rx="1.5" fill="white" />
                      </svg>
                      </div>
                        : null}
                    </div>
