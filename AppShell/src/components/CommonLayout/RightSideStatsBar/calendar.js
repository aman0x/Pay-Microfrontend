  ...
  return (
    <div className="flex justify-between  relative">
      <Calender
        ...
        prevLabel={
          <MdChevronLeft size="16px" className="calendar-btn rounded-[50%]" />
        }
        nextLabel={
          <MdChevronRight size="16px" className="calendar-btn rounded-[50%]" />
        }
        prev2Label={null}
        next2Label={null}
      />
      ...
      {isCalenderSheetOpen ? (
        <div
          className="absolute right-5 top-[18px] rounded-[50%] p-1 bg-primary calendar-btn"
          onClick={() => setCalenderSheet(false)}
        >
          <svg
            width="12"
            height="12"
            ...
            {months.map((month, i) => {
              return (
                <div
                  ...
                  key={i}
                >
                  <div className={`${monthIndex === i ? "poppins-bold" : ""}`}>
                    {month}
                  </div>
                  <div
                    className={`max-w-[15px]  max-h-[15px] rounded-sm   ${
                      monthIndex === i ? "primary-linear-gr-bg" : "bg-gray-300"
                    }`}
                  >
                    <FaSquare
                      color={`${monthIndex === i ? "black" : "white"}`}
            ...
            {years.map((year, i) => {
              return (
                <div
                  ...
                  key={i}
                >
                  <div className={`${yearIndex === i ? "poppins-bold" : ""}`}>
                    {year}
                  </div>
                  <div
                    className={`max-w-[15px]  max-h-[15px]  rounded-sm  ${
                      yearIndex === i
                        ? "primary-linear-gr-bg-up"
                        : "bg-gray-300"
                    }`}
                  >
                    <FaSquare
                      color={`${yearIndex === i ? "black" : "white"}`}
                      className=" rounded-sm p-[1px]"
