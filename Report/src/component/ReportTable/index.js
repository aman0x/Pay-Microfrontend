            ...
            <div className="w-full  bg-white p-[1.5rem] rounded-2xl">
                <div className="flex flex-col gap-1 mt-4" >
                    ...
                    <div className="flex gap-2 items-center">
                        <div className="poppins-light text-sm text-[#B6B8BA]">Transactions details:</div>
                        <div className="poppins-semibold text-sm">01 - 31 March 2024</div>
                    </div>
                </div>
                <table className="w-full  rounded-2xl overflow-hidden mt-7" id="report-content">
                    <thead>
                        <tr className="poppins-light text-xs text-white bg-black-primary items-center ">
                            <td>
                                <div className="flex items-center gap-2 td-element1 ml-2 ">
                                    <div className={`w-[13px]  h-[13px] rounded-[4px] ${isDateClicked ? 'primary-linear-gr-bg' : 'bg-white border-2'}`} onClick={() => { setIsDateClicked(!isDateClicked) }}>
                                    ...
                                    <tr className="text-xs poppins-regular">
                                        <td>
                                            <div className="flex items-center gap-3 td-element">
                                                <div className={`w-[12px]  h-[12px] rounded-[4px] ${isDateClicked || isRowSelected(i) ? 'primary-linear-gr-bg' : 'bg-white border-2'}`}
                                                    onClick={(e) => {
                                                        e.stopPropagation()
                                                        handleRowClick(i)
                                                    }}>
                                        ...
                                        <td><div className="td-element">{transaction?.beneficiary_bank_account?.bank_name}</div></td>
                                        <td><div className="td-element">{transaction?.beneficiary_bank_account?.bank_type || "Individual"}</div></td>
                                        <td><div className="td-element">{transaction.transaction_number}</div></td>
                                        <td>
                                            ...
                                            <div className="flex gap-2 items-center td-element">
                                                ...
                                                <div>
                                                    <svg width="9" height="9" viewBox="0 0 9 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <circle cx="4.5" cy="4.5" r="4.5" fill={`${transaction.transaction_status === 'completed' ? '#27A963' : '#FFB442'}`} />
                                                    </svg>
                                                </div>
